...
const renderData = (data) => {
    ...
    data.forEach((elem) => {
        // if (elem.old_price != 0) {
        //     const discount = Math.round(100 - elem.price * 100 / elem.old_price);
        // }
        ...
        // (num == 5) ?  console.log('Ye') :  console.log('no');
        // (num == 5) ?  res = 5 : res = 0;
...
const addToLS = (productId) => {
    ...
    let arr = [];
    if (cartFromLS) {
        arr = JSON.parse(cartFromLS); // [ {idUser: 1, data: [3,4,5]}, {idUser: 2, data: [3,4,5]} ]
